...
class G_Clusteron_XOR(gcl.G_Clusteron_Parent):
    def __init__(self,train_set,train_Y,test_set,test_Y,posVal,#deleted bias,
                 radius,init_locations = 'normal_f',
                 save_folder=None,hist_folder=None,weights = False):
        self.initial_locations = init_locations
        super().__init__(train_set,train_Y,test_set,test_Y,posVal,#refers to parent class
    ...
    def init_locations(self):
        ...
        if self.initial_locations == 'normal_f_converge':#this was done to only get initialization that are expected to converge
            locations = np.zeros(np.shape(self.train_set)[1])
            random_f = np.random.random(1)
            locations[1] = np.sqrt(-(self.radius)*np.log(random_f))
    ...
    def train(self,epochs,learning_protocol, batch_size, location_lr, weight_lr, bias_lr,
              W_momentum=True, B_momentum = True, L_momentum = True,test_epoch = 1):
        accuracy_vec = []
        for epoch in range(epochs):
            ...
            accuracy = self.test(self.test_set,
                                 self.binary_test_Y)#,confusion_matrix=plot_confusion_M,
