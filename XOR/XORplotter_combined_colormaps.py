'''
XOR plotter for data in json files - Colormap version
Shows both initial and final weights using different matplotlib colormaps for each panel
Each panel's entire aesthetic is based on its assigned colormap
'''
import os
import sys
...
def create_combined_scatter_plots(json_files,epochs=10000,fig_size = (7.5,6.)):
    ...
    fig = plt.figure('Combined XOR Visualization - Colormaps', facecolor='white')
    ...
    figure_filename = f'figures/XOR_combined_colormaps_{timestamp}.png'
    try:
    ...
    parameters = {
        ...
        'fig_size': fig_size,
        'color_schemes': [
            {'name': 'Plot 1 - Red/Blue/Green', 'conv': 'red', 'non_conv': 'blue', 'non_conv_exp': 'green'},
            {'name': 'Plot 2 - Purple/Orange/Brown', 'conv': 'purple', 'non_conv': 'orange', 'non_conv_exp': 'brown'},
            {'name': 'Plot 3 - Pink/Cyan/Black', 'conv': 'pink', 'non_conv': 'cyan', 'non_conv_exp': 'black'},
            {'name': 'Plot 4 - Yellow/Magenta/Navy', 'conv': 'yellow', 'non_conv': 'magenta', 'non_conv_exp': 'navy'},
            {'name': 'Plot 5 - Lime/Coral/Gray', 'conv': 'lime', 'non_conv': 'coral', 'non_conv_exp': 'gray'},
            {'name': 'Plot 6 - Gold/Teal/Maroon', 'conv': 'gold', 'non_conv': 'teal', 'non_conv_exp': 'maroon'}
        ],
        'panel_backgrounds': ['lightcoral', 'lightsteelblue', 'lightgreen', 'lightyellow', 'lightpink', 'lightgray'],
        'cube_colors': ['mistyrose', 'aliceblue', 'honeydew', 'ivory', 'lavenderblush', 'azure'],
        'viewing_angles': [(20, -135), (30, -45), (15, 45), (25, -90), (35, 0), (20, 135)],
        'style': 'Improved Colors from Commit - Bright and Distinct'
    }
    
    parameters_filename = f'figures/XOR_colormaps_parameters_{timestamp}.json'
    try:
