#calculate delta locations for a dataset
import numpy as np
...
def calculate_Q_matrix(example,weights):
    ...
    """
    #assert np.shape(example)[0] == 1, 'error Q input, bad dimensions'
...
##    print('F = ',F_matrix)
    ...
    #synapse_average = np.mean(activations,0)
    #print('activations = ',activations)
    return activations #,synapse_average
...
def calculate_delta_locations(F_matrix,D_matrix, dataset, weights, L_minus_y):
    ...
    #print('delta locations =',delta_locations)
...
def sanity_check_delta_locations(dataset, weights,locations,radius,error):#add for activations seperate function
    ...
    #print('sanity check delta locations = ',delta_locations)
    return delta_locations
...
def sanity_check_activations(dataset, weights, locations,radius):
    ...
    #print('sanity check activations = ',activations)
    return
...
# x = np.reshape(np.arange(N*P),(P,N))
# w = np.linspace(0.1,1,N)
# l = np.random.randint(-3,3,N)
# error = np.random.randint(2,5,P)
# z = 1
# radius = -(z**2)/np.log(0.5)
# 
# ##x = np.matrix([[1,3,-5],[1,3,-5]])
# ##x1 = x[0] 
# ##w1 = np.array([0.1,0.2,0.1])
# 
# S = calculate_S_matrix(x, w)
# D = calculate_D_matrix(l)
# F = calculate_F_matrix(D, radius)
# Q_ewam = calculate_Q_error_weighted_avg_matrix(x,w,error)
# A,sn,eo = calculate_activations(S, F)
#sanity_check_activations(x, w, l,radius)
#sanity_check_delta_locations(x,w,l,radius,error)
#rr = calculate_delta_locations(F,D,Q_ewam)
#calculate_delta_weights(A,w,error)
