...
class G_Clusteron_Parent():
    ...
    def __init__(self,train_set,train_Y,test_set,test_Y,posVal,radius, 
        ...
        self.num_of_syn = np.shape(self.train_set)[1]#changed this from len to np.shape for toy model
        self.radius = radius
        self.init_distance_scale = init_distance_scale
        self.locations = self.init_locations()#/100 was a good default
        self.bias = 0
        self.delta_bias = 0
        self.m_locations = 0
        self.v_locations = 0
    ...
    def train_epoch(self, epoch_num,learning_protocol, batch_size, location_lr, weight_lr,
        ...
        self.calculate_D_matrix()#distance,G,and delta locations matrices
        self.calculate_F_matrix()
##        if epoch_num == 0:##to get initial activation data
##            self.initial_activations = self.calculate_activations_by_dataset(self.test_set)
